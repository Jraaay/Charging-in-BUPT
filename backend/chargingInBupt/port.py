...
async def end_charging_request(request):
    ...
    request = session.query(ChargeRequest).filter(ChargeRequest.user_id == user.id).last()
    if request is None:
        success = False
        error_msg = "该用户没有充电请求"
    else:
        ...
        # 如果当前正在充电，计算后再创建充电详单 question：算法尚未测试
            ...
            begin_time = datetime.datetime.strptime(request.start_time, "%Y-%m-%d %H:%M:%S")
            end_time = now
            ...
            for i in range(len(clocks)):
                if clocks[i] <= begin_time.hour < clocks[i + 1]:
                    begin_time_zone = i + 1
                if clocks[i] <= end_time.hour < clocks[i + 1]:
                    end_time_zone = i + 1
            # 如果开始和结束的时间区域相同
            if begin_time_zone == end_time_zone:
                charging_cost = '%.2f' % (charged_amount * fees[begin_time_zone - 1])  # 充电费用
            else:
                # 分别计算开始时间到临界值的时间，结束时间到临界值的时间。单位为秒
                diff_time1 = (clocks[begin_time_zone + 1] - begin_time.hour - 1) * 3600 + \
                             (60 - begin_time.minute) * 60 + 60 - begin_time.second
                diff_time2 = (end_time.hour - clocks[end_time_zone]) * 3600 + end_time.minute * 60 + end_time.second
                ...
                for i in zones:
                    if i == begin_time_zone:
                        charging_cost = diff_time1 / 3600 * rate * fees[begin_time_zone - 1]
                    elif i == end_time_zone:
                        charging_cost += diff_time2 / 3600 * rate * fees[begin_time_zone - 1]
                    else:
                        charging_cost += (clocks[i + 1] - clocks[i]) * rate * fees[begin_time_zone - 1]

            charging_cost = '%.2f' % charging_cost  # 充电费用
            ...
            # 更新充电桩信息
            charger = session.query(Charger).fliter(Charger.id == request.charge_pile_id).first()
            charger.cumulative_charging_time += charged_time
            charger.cumulative_charging_amount += charged_amount
